...
denz.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [stod]},quoted:finv})
break
case 'menu': 
stod = `${sender}`
...
Link : ${get_resultP.url_audio}
			...
			case 'runtime':
				...
				denz.sendMessage(from, teks.runtime(), extendedText, { quoted: mek, contextInfo: { mentionedJid: [stod]},quoted:fvid})
				break
			case 'setbio':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
				if (args.length < 1) return reply('Teksnya?')
